...
<div class="LoginWindow">
	<div>
		<div class="login">
			...
			<p class="login-submit"><button type="submit" class="login-button" onclick="login()" id ="submit">登录</button></p>
		</div>
...
function login(){
	...
	$.ajax({
		url:"<%=request.getContextPath()%>/user/querylogin.action",
		type:"post",
		data:{
			"username":username,
			"userpass":userpass,
		  ...
		  success:function(fang){
			idsStr = fang.substring(1,fang.length-1);
			 ...
			 if(idsStr=="success"){
				 ...
				 location.href="<%=request.getContextPath()%>/user/queryloginsuccess.action";
		       
				  }else if(idsStr=="error"){
					  alert("ç¨æ·åæææºå·ä¸å­å¨"); 
